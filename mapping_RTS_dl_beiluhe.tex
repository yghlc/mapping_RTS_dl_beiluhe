...
Similar, their IOU values of mapped polygons (a histogram in Fig. S17) are not as high as the ones in the study area.
Compared with the experiment results in the study area, both the percentage of false positives and false negatives are higher, leading to a low F1 score. 
The reason for the lower F1 score in the test area is that some of RTSs or land cover with different appearance were not included in the training data. %the image is different as well.
...
Fig. S16e is a case that the mapping method only picked a portion of an RTS, indicating that the features of this RTS is not well learned by the trained model.}
