...
Training is an iterative process that makes the DeepLabv3+ network learn features from training data. The network architecture we used was Xception65 \citep{chollet2017xception}, and the pre-trained model was based on the ImageNet datasets \citep{russakovsky2015imagenet}. The learning rate was 0.007, \added{the batch size was eight, }and the iteration number was 30000, as recommended by the original study of DeepLabv3+ \citep{chen_encoder-decoder_2018}. \added{We chose the best-trained model based on the value of the average precision (section \ref{subsubsec_validation}).} To evaluate the robustness of this algorithm, we utilized \added{3-fold, }5-fold, and 10-fold cross-validation, which are commonly used in machine learning. In the \replaced{3}{5}-fold cross-validation, we randomly divided the positive and negative training polygons into \replaced{three}{five} portions with a roughly equal count, and then used \replaced{two}{four} portions for training and the remaining one for validation. We repeated this training process five times and present the results in Fig. S4 in the Supplementary Materials. Fig. S5 \added{and S6} show\deleted{s} that average precision in \added{5-fold and} 10-fold cross-validation \deleted{is} slightly \added{increase}\deleted{higher than the one in 5-fold}, which indicates that more training data (90\% in 10-fold\replaced{,}{and} 80\% in 5-fold\added{, 67\% in 3-fold}) can lead to better results. To achieve the best results, we included all the ground truths in the training polygons. 
...
\subsection{Prediction accuracies in a new area}
\label{subsec_newArea}
% result in a distinct geographic regions
% Test accuracies in a new area?

\added{A test in a new area achieved good accuracies by using the trained model in experiment \#22. %regarding the training process and collecting training data. 
The new area with a size of approximately 4000 km$^2$ is to the northwestern of our study area, as shown in Fig. S15a in the Supplementary Materials. 
This test area was chosen randomly because we did not know RTS distribution outside the Beiluhe region on the Tibetan Plateau.
The image used for predicting RTSs is a mosaic of 47 Planet images acquired on 22 and 23 May 2018. 
We used the same method described in section \ref{subsec_collect_groundtruth} to manually delineate RTSs in this area and obtained 50 RTS polygons (i.e., ground truths).
As shown in Fig. S15b and S16, the mapping method delineated 62 mapped polygons. 
Validating against the ground truths by setting the IOU threshold as 0.5 shows that 33 and 20 of them are true and false positives, respectively.
The number of false negatives is 17, indicating that 17 of the RTSs are missed by the mapping method. 
Therefore, the precision, recall, and F1 score are 0.532, 0.660, and 0.589, which shows a good performance of the mapping method in a new area.}

\added{The accuracy in the test area is lower than the one in the study area.  
All the experiments in Table \ref{table_acc_imgaug} and S1 show that the F1 score of mapped polygons in the study area is higher than the one in the test area by around 0.3. 
%As presented in the Table 1, although the two areas have different extents and RTS number.  
The boundaries of most mapped polygons match the ones of ground truths, but not as well as the case in the study area, as suggested by some true positives in Fig. S16a and b. 
Similarly, their IOU values (a histogram in Fig. S17) are not as high as the ones in the study area.
Compared with the experiment results in the study area, both the percentage of false positives and false negatives are higher, leading to a lower F1 score. 
The reason for the lower F1 score in the test area is that some of RTSs or land cover with different appearances on images were not included in the training data. %the image is different as well.
For example, Fig. S16c shows two false negatives which are darker than most RTSs in the study area on Planet images. 
Fig. S16a, and d show examples of false positives which have similar appearances of RTSs in the study area. 
Fig. S16e is a case that the mapping method only picked a portion of an RTS, indicating that the features of this RTS are not well learned by the trained model.}
...
\added{The possible reason for the small mapped polygons with high IOU values is that they shared similar features which were enhanced during training and were easier to be detached during prediction.}
...
The geometric characteristics of the mapped RTSs are similar to those \replaced{featured in the ground truth data set}{ featuring as ground truths}. Fig. \replaced{S7}{S6} in the Supplementary Materials shows that the averages of the areas, perimeters, circularities of the ground truths 
...
\added{Great thanks to editor Joseph Awange for handling this paper and three anonymous reviewers for their constructive suggestions and insightful comments.}
...
\added{And also thanks Professor Thian Yew Gan for hosting Lingcao Huang's visit to Canada and discussion on the results.}
